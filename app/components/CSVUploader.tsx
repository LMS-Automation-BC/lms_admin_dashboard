...
const CsvUpload: React.FC = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    return groupedData.filter(item => item.AbsentDates.includes(selectedDate));
      ...
      {groupedData.length > 0 && (
        <>
          <div className="date-filters">
            <button
              className={`date-filter-btn ${selectedDate === null ? 'active' : ''}`}
            ...
            {uniqueDates.map(date => (
              <button
                key={date}
                className={`date-filter-btn ${selectedDate === date ? 'active' : ''}`}
          ...
          <table>
            <thead>
              <tr>
                ...
                <th>ID</th>
                <th>AbsentDates</th>
                <th>User Email</th>{/* example extra info from API */}
              </tr>
            </thead>
            <tbody>
